...
// Items table's required-column floor -- even after that floor was
// trimmed from 848px to 768px (see useItemsTableResponsive.ts), 8/12 of an
// 1100px window is only 733px. Widening the split to 9/12 (825px at
// 1100px) closes it, applied only in this band via an inline :style
// override so it doesn't touch the split ratio anywhere else.
describe("POS layout: narrow-band split override (1100-1272px)", () => {
	...
	it("the 9/12 override at the bottom of the band (1100px) clears the trimmed required-columns floor", () => {
		...
		// agree. The 768px figure (200 + 116 + 82 + 82 + 92 + 88 + 60 data
		...
		// override is actually load-bearing here, not redundant.
		...
		const headers = [
			...
			{ key: "discount_percentage", title: "Discount %", required: true },
			{ key: "discount_amount", title: "Discount Amount", required: true },
